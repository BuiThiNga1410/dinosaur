...
class Order {
  getAllOrder(req, res) {
    ...
    const options = {
      ...
      populate: [{
        ...
        select: "name images minPrice maxPrice discount quantity ratingsReviews"
      },
      {
        path: "user",
  ...
  getOrderByUser(req, res) {
    ...
    if (!userId) {
      ...
      orderModel.find({ user: userId })
        ...
        .populate("products.product", "name images minPrice maxPrice discount quantity ratingsReviews")
        .then(orders => {
  ...
  async createOrder(req, res) {
    const { user, name, phone, address, ward, district, province, products, price, shippingFee, shippingId, typePay } = req.body;
    if (!user || !name || !phone || !address || !province || !district || !ward || !products.length || !price || !shippingId || !typePay) {
            ...
            .then(() => {
              try {
                ...
                products.forEach(async item => {
                  ...
                  const result1 = await productModel.findByIdAndUpdate(item.product, {
                    $set: {
  ...
  confirmOrder(req, res) {
    const { id } = req.body;
    orderModel.findByIdAndUpdate(id, { $set: { status: 'Confirmed' } })
      .exec()
      .then((result) => {
