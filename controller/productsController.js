...
class Product {
  ...
  addProduct(req, res) {
    const { name, images, category, description, skus, quantity, discount, status, minPrice, maxPrice, sold, weight, length, height, width } = req.body;
    if (!name || !category || !description || !minPrice || !maxPrice || !weight || !length || !width || !height) {
  ...
  deleteProduct(req, res) {
    const id = req.params.productId;
    productModel.findByIdAndUpdate(id, { status: false })
      .exec()
      .then(() => {
        res.status(204).json({
          success: true
  ...
  async postAddReview(req, res) {
    let { userId, productOrderId, productId, orderId, rating, review } = req.body;
    if (!userId || !rating || !review || !productOrderId || !orderId || !productId) {
        ...
        newRatingReview.exec((err, result) => {
          if (err) {
            ...
            orderModel.findById(orderId)
              .then((order) => {
                const indexProduct = order.products.findIndex((item) => String(item._id) === String(productOrderId));
