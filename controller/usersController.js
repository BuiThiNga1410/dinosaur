...
class User {
  async getAllUser(req, res) {
    try {
      let users = await userModel
        .find({ userRole: 0 })
        .select("_id name email phone dob address gender userImage status updatedAt createdAt");
      if (users) {
    ...
    if (!userId) {
      ...
      try {
        let user = await userModel
          .findById(userId)
          .select("_id name email phone dob address gender userImage updatedAt createdAt");
        if (user) {
  ...
  async deleteUser(req, res) {
    ...
    userModel.findByIdAndUpdate(id, { status: false })
      .exec()
      .then(() => {
        res.status(200).json({
          success: true
